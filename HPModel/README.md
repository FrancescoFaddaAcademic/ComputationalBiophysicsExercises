...
Proteins have an immense number of degrees of freedom, so many in fact, that it would be impossible for them to explore a significant portion of their phase space in a reasonable time. The study of how protein manage to foldi into functioning structures is an area of great interest activity. 

One particular class of proteins that are particularly stable in their ground state is that of globular proteins, proteins whose conformation is well localized and very compact. These proteins have the interesting property that different regions of the chain are forced in the center or on the outskirts of the protein according to them being Hydrophobic or Polar (respectively). It turns out that this kind of behavior is responsible for the great reduction in the number of configurations accessed by the protein. 
The Hydrophobic-Polar (HP) model is a toy model used to understand how the polar characteristics of a protein affect its folding and conformation, it abstracts away the complex structure of the protein and cosiders only their affinity to water to gather information on the statistics of their configurations. In the context of the HP model a protein can be understood as a path on a 2 or 3 dimensional lattice. Each residue is stripped of all the specific structure and can be thought of as a binary label containing (H) for hydrophobic and (P) for polar. On this structure one can perform some interesting calculations, in particular various models can be used to measure the NN energy of the configuration, the compactness can readily be calculated as the number of non connected neighbors, and with both this data, one can measure the degeneracy of various energy states.   
